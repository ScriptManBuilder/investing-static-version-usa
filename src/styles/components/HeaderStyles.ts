...
export const Logo = styled(Link)`
  ...
  @media (max-width: 480px) {
    font-size: 0.8rem;
    line-height: 1.1;
    max-width: 140px; /* Увеличиваем ширину для DATA VISUALIZATION */
  }
...
export const NavLinks = styled.div<{ isOpen: boolean }>`
  ...
  @media (max-width: 768px) {
    position: fixed;
    top: ${props => props.isOpen ? '95px' : '-300px'};
    right: 15px;
  ...
  @media (max-width: 480px) {
    top: ${props => props.isOpen ? '80px' : '-250px'};
    right: 10px;
    padding: 16px;
    min-width: 180px;
    gap: 6px;
  ...
  @media (max-width: 375px) {
    top: ${props => props.isOpen ? '75px' : '-200px'};
    right: 8px;
    padding: 14px;
    min-width: 160px;
    gap: 5px;
  ...
  @media (max-width: 320px) {
    top: ${props => props.isOpen ? '70px' : '-180px'};
    right: 6px;
    padding: 12px;
    min-width: 140px;
    gap: 4px;
...
export const ProductsNavItem = styled.div`
  ...
  @media (max-width: 768px) {
    ...
    /* Полностью переопределяем стили для NavLink внутри */
    & a {
      display: block !important;
      width: 100% !important;
      text-align: center !important;
      ...
      /* Убираем все градиенты и специальные стили */
